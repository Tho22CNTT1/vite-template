...
export const createProduct = async (req, res, next) => {
    try {
        const { product_name, price, stock, category_id, brand_id, discount, model_year, description, slug, } = req.body;
        if (!product_name)
            return next(createError(400, "product_name is required"));
        if (!category_id)
            return next(createError(400, "category_id is required"));
        if (!brand_id)
            return next(createError(400, "brand_id is required"));
        ...
        if (Number.isNaN(priceNum))
            return next(createError(400, "price must be a number"));
        if (Number.isNaN(stockNum))
            return next(createError(400, "stock must be a number"));
        if (discount !== undefined && Number.isNaN(discountNum))
            return next(createError(400, "discount must be a number"));
        if (model_year !== undefined && Number.isNaN(modelYearNum))
            return next(createError(400, "model_year must be a number"));
...
export const updateProduct = async (req, res, next) => {
    try {
        ...
        if (b.product_name !== undefined)
            product.product_name = b.product_name;
        ...
        if (b.category_id !== undefined)
            product.category_id = new Types.ObjectId(b.category_id);
        if (b.brand_id !== undefined)
            product.brand_id = new Types.ObjectId(b.brand_id);
